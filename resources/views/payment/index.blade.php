        ...
        <div class="col-lg-6 col-md-8">
            <div class="booking-card">
                <div class="text-center mb-4">
                    ...
                    <p class="text-muted">Selesaikan pembayaran untuk mengonfirmasi reservasi Anda</p>
                </div>
                ...
                <div class="confirmation-box mb-4">
                    ...
                    <div class="confirm-row">
                        <span>Area/Meja</span>
                        <strong>{{ $reservasi->area->nama_area }} {{ $reservasi->meja ? '- ' . $reservasi->meja->kode_meja : '' }}</strong>
                    </div>
                    <div class="confirm-row">
                        <span>Waktu</span>
                        <strong>{{ \Carbon\Carbon::parse($reservasi->tanggal_reservasi)->format('d M Y') }} | {{ \Carbon\Carbon::parse($reservasi->jam_reservasi)->format('H:i') }} - {{ \Carbon\Carbon::parse($reservasi->jam_selesai)->format('H:i') }}</strong>
                    </div>
                    
                    <h6 class="mt-4 mb-2">Detail Menu:</h6>
...
<script>
    ...
    buktiBayar.addEventListener('change', function(e) {
        ...
        if (file.type === 'application/pdf') {
            // For PDF, show file name instead of image
    ...
    btnPay.onclick = function() {
        if (!selectedMethod || !selectedFile) {
            Swal.fire({
                icon: 'warning',
                title: 'Perhatian',
                text: 'Silakan pilih metode pembayaran dan upload bukti pembayaran terlebih dahulu'
            });
            return;
        }
