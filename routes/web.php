...
Route::get('/semua-menu', function () {
    ...
    $menus    = $kategori
        ? \App\Models\Menu::where('kategori', $kategori)->get()
        : \App\Models\Menu::all();
...
/*
|--------------------------------------------------------------------------
| CUSTOMER — requires auth:web (bukan auth default)
|--------------------------------------------------------------------------
*/
Route::middleware('auth:web')->group(function () {
    ...
    Route::get('/booking/confirmation/{id}', [ReservasiController::class, 'confirmation'])->name('booking.confirmation');
...
Route::prefix('admin')->group(function () {
    ...
    Route::post('/forgot-password', [AuthController::class, 'resetDirectPassword'])->name('admin.password.update');

});

// Route admin yang butuh auth:admin
Route::prefix('admin')->middleware('auth:admin')->group(function () {

    // DASHBOARD
    Route::get('/dashboard', [DashboardController::class, 'index'])->name('admin.dashboard');
    Route::get('/dashboard/latest-reservasi', [DashboardController::class, 'latestReservasi'])->name('admin.dashboard.latest');
    Route::get('/search', [DashboardController::class, 'search'])->name('admin.search');

    // RESERVASI
    Route::get('/reservasi',         [ReservasiAdminController::class, 'index'])->name('admin.reservasi');
    Route::get('/reservasi/json',    [ReservasiAdminController::class, 'getJson'])->name('admin.reservasi.json');
    Route::post('/reservasi/{id}/status', [ReservasiAdminController::class, 'updateStatus'])->name('admin.reservasi.status');
    Route::delete('/reservasi/{id}', [ReservasiAdminController::class, 'destroy'])->name('admin.reservasi.destroy');
    ...
    Route::delete('/area/{id}',       [AreaController::class, 'destroy'])->name('admin.area.delete');

    // MEJA
    Route::get('/meja',               [MejaController::class, 'index'])->name('admin.meja');
    ...
    Route::get('/meja/edit/{id}',     [MejaController::class, 'edit'])->name('admin.meja.edit');
    Route::post('/meja/update/{id}',  [MejaController::class, 'update'])->name('admin.meja.update');
    Route::delete('/meja/{id}',       [MejaController::class, 'destroy'])->name('admin.meja.destroy');

    // MENU
    Route::get('/menu',               [MenuAdminController::class, 'index'])->name('admin.menu');
    Route::get('/menu/create',        [MenuAdminController::class, 'create'])->name('admin.menu.create');
    Route::post('/menu',              [MenuAdminController::class, 'store'])->name('admin.menu.store');
    Route::get('/menu/{id}/edit',     [MenuAdminController::class, 'edit'])->name('admin.menu.edit');
    Route::put('/menu/{id}',          [MenuAdminController::class, 'update'])->name('admin.menu.update');
    Route::delete('/menu/{id}',       [MenuAdminController::class, 'destroy'])->name('admin.menu.destroy');
    Route::patch('/menu/{id}/toggle', [MenuAdminController::class, 'toggle'])->name('admin.menu.toggle');

    // GALERI
    Route::get('/galeri',             [GaleriAdminController::class, 'index'])->name('admin.galeri');
    Route::get('/galeri/create',      [GaleriAdminController::class, 'create'])->name('admin.galeri.create');
    Route::post('/galeri',            [GaleriAdminController::class, 'store'])->name('admin.galeri.store');
    Route::delete('/galeri/{id}',     [GaleriAdminController::class, 'destroy'])->name('admin.galeri.destroy');
    ...
    Route::get('/notifikasi', function () {
        $data = \App\Models\Reservasi::where('notif_dibaca', false)->latest()->take(5)->get();
        return response()->json(['count' => $data->count(), 'data' => $data]);
    })->name('admin.notifikasi');

    Route::post('/notifikasi/read/{id}', function ($id) {
        \App\Models\Reservasi::where('id_reservasi', $id)->update(['notif_dibaca' => true]);
